...
splash_title <- function(header, content, button = TRUE, button_label,img_loc,colorr=rgb(121/255,28/255,153/255)){
  rgb <- hex_rgb(colour)
  if(button){
    div(class = "jumbotron",style=sprintf("height: 400px; background: linear-gradient(rgba(%s,%s,%s, 0.45),rgba(%s,%s,%s, 0.45)),url('%s');background-repeat: no-repeat;background-position: 50%% 50%%;background-size: cover;",rgb$r,rgb$g,rgb$b,rgb$r,rgb$g,rgb$b,img_loc),
